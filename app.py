...
def process_uploaded_file(contents, start_date, end_date, range_values, filename):
    if contents is not None:
        cached_result = cache.get('processed_data')  # Try to retrieve cached result

        if cached_result is not None:
            # If cached result exists, return it
            output_message, table_data = cached_result
            ...
            cache_output_message, cache_filtered_data = output_message, filtered_data

            cache.set('processed_data', (cache_output_message, cache_filtered_data))
